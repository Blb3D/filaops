    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">Track and manage payment transactions</p>
        </div>
      ...
      {dashboard && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4">
            ...
            <div className="text-2xl font-bold text-white">{formatCurrency(dashboard.amount_today)}</div>
            <div className="text-sm text-gray-500">{dashboard.payments_today} payment{dashboard.payments_today !== 1 ? "s" : ""}</div>
          </div>

          {/* This Week */}
          <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4">
            <div className="text-gray-400 text-xs uppercase tracking-wide mb-1">This Week</div>
            <div className="text-2xl font-bold text-white">{formatCurrency(dashboard.amount_this_week)}</div>
            <div className="text-sm text-gray-500">{dashboard.payments_this_week} payment{dashboard.payments_this_week !== 1 ? "s" : ""}</div>
          </div>

          {/* This Month */}
          <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4">
            <div className="text-gray-400 text-xs uppercase tracking-wide mb-1">This Month</div>
            <div className="text-2xl font-bold text-green-400">{formatCurrency(dashboard.amount_this_month)}</div>
            <div className="text-sm text-gray-500">{dashboard.payments_this_month} payment{dashboard.payments_this_month !== 1 ? "s" : ""}</div>
          </div>

          {/* Outstanding */}
          <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4">
            <div className="text-gray-400 text-xs uppercase tracking-wide mb-1">Outstanding</div>
            <div className="text-2xl font-bold text-yellow-400">{formatCurrency(dashboard.total_outstanding)}</div>
            <div className="text-sm text-gray-500">{dashboard.orders_with_balance} order{dashboard.orders_with_balance !== 1 ? "s" : ""}</div>
          </div>
        </div>
      )}

      {/* Payment Methods Breakdown */}
      {dashboard && dashboard.by_method && Object.keys(dashboard.by_method).length > 0 && (
        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 mb-6">
          <div className="text-gray-400 text-xs uppercase tracking-wide mb-3">This Month by Method</div>
          <div className="flex flex-wrap gap-4">
            {Object.entries(dashboard.by_method).map(([method, amount]) => (
              <div key={method} className="flex items-center gap-2">
                <span className="text-gray-400">{paymentMethodLabels[method] || method}:</span>
            ...
            <table className="w-full">
              ...
              <tbody>
                {payments.map((payment) => (
                  <tr key={payment.id} className="border-t border-gray-800 hover:bg-gray-800/50">
                    ...
                    <td className="py-3 px-4 text-gray-400">
                      ...
                      {payment.check_number && (
                        <span className="text-gray-500 text-xs ml-1">#{payment.check_number}</span>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded text-xs ${
                        payment.payment_type === "refund" ? "bg-red-500/20 text-red-400" : "bg-blue-500/20 text-blue-400"
                      }`}>
                        {payment.payment_type}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded text-xs ${statusColors[payment.status] || "bg-gray-500/20 text-gray-400"}`}>
                        {payment.status}
                      </span>
                    </td>
                    ...
                    <td className="py-3 px-4 text-right">
                      {payment.status === "completed" && (
                        <button
                          onClick={() => handleVoidPayment(payment.id, payment.payment_number)}
            ...
            {pagination.totalPages > 1 && (
              <div className="flex justify-between items-center px-4 py-3 border-t border-gray-800">
                <div className="text-sm text-gray-500">
                  Showing {((pagination.page - 1) * 25) + 1} to {Math.min(pagination.page * 25, pagination.total)} of {pagination.total}
                </div>
                <div className="flex gap-2">
                  <button
