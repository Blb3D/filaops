...
async def get_analytics_dashboard(
    days: int = 30, current_user: User = Depends(get_current_admin_user), db: Session = Depends(get_db)
):
    """
    ...
    revenue_query = db.query(func.sum(SalesOrder.total_price).label("total")).filter(SalesOrder.status == "completed")

    total_revenue = revenue_query.scalar() or Decimal("0")
    revenue_30 = revenue_query.filter(SalesOrder.created_at >= end_date - timedelta(days=30)).scalar() or Decimal("0")
    revenue_90 = revenue_query.filter(SalesOrder.created_at >= end_date - timedelta(days=90)).scalar() or Decimal("0")
    revenue_365 = revenue_query.filter(SalesOrder.created_at >= end_date - timedelta(days=365)).scalar() or Decimal("0")
    ...
    total_customers = db.query(func.count(User.id)).filter(User.account_type == "customer").scalar() or 0
    ...
    top_customers_query = (
        db.query(User.company_name, User.id, func.sum(SalesOrder.total_price).label("revenue"))
        .join(SalesOrder)
        .filter(SalesOrder.status == "completed", SalesOrder.created_at >= start_date)
    ...
    top_customers = [
        {"customer_id": c.id, "company_name": c.company_name or "N/A", "revenue": float(c.revenue)}
    ...
    # Product metrics
    total_products = db.query(func.count(Product.id)).filter(Product.active.is_(True)).scalar() or 0
    ...
    top_products = [
        {"sku": p.sku, "name": p.name, "quantity_sold": float(p.qty_sold), "revenue": float(p.revenue)}
    ...
    # Profit metrics
    total_cost = db.query(func.sum(SalesOrderLine.quantity * Product.standard_cost)).join(Product).join(
        SalesOrder
    ).filter(SalesOrder.status == "completed", SalesOrder.created_at >= start_date).scalar() or Decimal("0")

    gross_profit = revenue_30 - total_cost
    gross_margin = float((gross_profit / revenue_30 * 100)) if revenue_30 > 0 else 0.0

    # Profit by product
    profit_by_product_query = (
        db.query(
            Product.sku,
            Product.name,
