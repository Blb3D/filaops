...
def find_or_create_customer(db: Session, email: str, name: str = None, shipping_address: dict = None) -> Optional[User]:
    ...
    last_customer = (
        db.query(User).filter(User.customer_number.like(f"CUST-{year}-%")).order_by(User.customer_number.desc()).first()
    )
        ...
        try:
            ...
            if not product_sku:
                result.errors.append(
                    {"row": row_num, "error": "Product SKU missing - line item skipped", "order_id": order_id}
                )
            ...
            orders_dict[order_id]["lines"].append({"sku": product_sku, "quantity": quantity, "unit_price": unit_price})

        except Exception as e:
            result.errors.append(
                {"row": row_num, "error": str(e), "order_id": order_id if "order_id" in locals() else ""}
            )
            result.skipped += 1
        ...
        try:
            ...
            if create_customers:
                customer = find_or_create_customer(
                    db, order_data["customer_email"], order_data["customer_name"], order_data["shipping_address"]
                )
            else:
                customer = db.query(User).filter(User.email.ilike(order_data["customer_email"])).first()
            ...
            for line in order_data["lines"]:
                ...
                if not product:
                    result.errors.append({"order_id": order_id, "error": f"Product not found: {line['sku']}"})
                    continue

                if not product.active:
                    result.errors.append({"order_id": order_id, "error": f"Product '{line['sku']}' is inactive"})
                    continue

                # Use provided price or product price
                unit_price = line["unit_price"] or product.selling_price or Decimal("0.00")
                if unit_price <= 0:
                    result.errors.append({"order_id": order_id, "error": f"Product '{line['sku']}' has no price"})
                    continue
            ...
            if not line_products:
                result.errors.append({"order_id": order_id, "error": "No valid products found for order"})
                result.skipped += 1
                continue

            # Check if order already exists (by source_order_id)
            existing_order = db.query(SalesOrder).filter(SalesOrder.source_order_id == order_id).first()

            if existing_order:
                result.errors.append(
                    {"order_id": order_id, "error": f"Order already exists: {existing_order.order_number}"}
                )
                result.skipped += 1
                continue
