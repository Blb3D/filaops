...
@router.get("/products")
async def export_products(current_user: User = Depends(get_current_staff_user), db: Session = Depends(get_db)):
    ...
    for p in products:
        ...
        writer.writerow(
            [
                ...
                p.procurement_type,
                p.unit,
                p.standard_cost or 0,
                p.selling_price or 0,
                on_hand,
                p.reorder_point or 0,
                p.active,
            ]
        )
...
async def export_orders(
    ...
    writer.writerow(["Order Number", "Customer", "Status", "Total", "Created Date", "Line Items"])
