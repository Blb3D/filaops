...
from app.schemas.mrp import (
    ...
    FirmPlannedOrderRequest,
    ReleasePlannedOrderRequest,
    ReleasePlannedOrderResponse,
    RequirementsSummary,
    NetRequirement,
...
async def run_mrp(
    request: MRPRunRequest, current_user: User = Depends(get_current_staff_user), db: Session = Depends(get_db)
):
    """
    Run MRP calculation.
...
async def list_mrp_runs(limit: int = Query(10, ge=1, le=100), db: Session = Depends(get_db)):
    """List recent MRP runs"""
    runs = db.query(MRPRun).order_by(MRPRun.run_date.desc()).limit(limit).all()

    # Find last successful run
    last_success = db.query(MRPRun).filter(MRPRun.status == "completed").order_by(MRPRun.run_date.desc()).first()

    return MRPRunSummary(runs=runs, last_successful_run=last_success.run_date if last_success else None)


@router.get("/runs/{run_id}", response_model=MRPRunResponse)
async def get_mrp_run(run_id: int, db: Session = Depends(get_db)):
...
async def list_planned_orders(
    ...
    total = query.count()
    orders = query.order_by(PlannedOrder.due_date).offset((page - 1) * page_size).limit(page_size).all()
    ...
    return PlannedOrderListResponse(items=items, total=total, page=page, page_size=page_size)


@router.get("/planned-orders/{order_id}", response_model=PlannedOrderResponse)
async def get_planned_order(order_id: int, db: Session = Depends(get_db)):
...
async def firm_planned_order(order_id: int, request: FirmPlannedOrderRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        order = service.firm_planned_order(planned_order_id=order_id, notes=request.notes)
        product = db.query(Product).get(order.product_id)
...
async def release_planned_order(order_id: int, request: ReleasePlannedOrderRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        order, created_id = service.release_planned_order(
            planned_order_id=order_id, vendor_id=request.vendor_id, notes=request.notes
        )

        response = ReleasePlannedOrderResponse(planned_order_id=order.id, order_type=order.order_type)
...
async def cancel_planned_order(order_id: int, db: Session = Depends(get_db)):
...
async def get_requirements(
    ...
    if production_order_id:
        ...
        requirements = service.explode_bom(
            product_id=product_id, quantity=1, source_demand_type="manual_query", source_demand_id=None
        )
    else:
        raise HTTPException(status_code=400, detail="Either product_id or production_order_id is required")
...
async def get_supply_demand_timeline(
    product_id: int, days_ahead: int = Query(30, ge=1, le=365), db: Session = Depends(get_db)
):
    """
    ...
    try:
        timeline = service.get_supply_demand_timeline(product_id=product_id, days_ahead=days_ahead)
...
async def explode_bom(product_id: int, quantity: float = Query(1.0, gt=0), db: Session = Depends(get_db)):
    """
    ...
    service = MRPService(db)
    requirements = service.explode_bom(
        product_id=product_id, quantity=Decimal(str(quantity)), source_demand_type="bom_explode", source_demand_id=None
    )
