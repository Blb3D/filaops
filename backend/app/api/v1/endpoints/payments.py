...
def update_order_payment_status(db: Session, order: SalesOrder):
    ...
    total_paid = db.query(func.coalesce(func.sum(Payment.amount), 0)).filter(
        Payment.sales_order_id == order.id, Payment.status == "completed"
    ).scalar() or Decimal("0")
...
def payment_to_response(payment: Payment) -> PaymentResponse:
    ...
    return PaymentResponse(
        ...
        recorded_by_name=f"{payment.recorded_by.first_name} {payment.recorded_by.last_name}".strip()
...
async def record_payment(
    ...
    if not order:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Sales order {payment_data.sales_order_id} not found"
        )
...
async def record_refund(
    ...
    if not order:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Sales order {refund_data.sales_order_id} not found"
        )
...
async def list_payments(
    ...
    """
    query = db.query(Payment).options(joinedload(Payment.sales_order), joinedload(Payment.recorded_by))
    ...
    payments = query.order_by(desc(Payment.payment_date)).offset((page - 1) * page_size).limit(page_size).all()
...
async def get_payment_dashboard(
    ...
    # Today's payments
    today_stats = (
        db.query(func.count(Payment.id), func.coalesce(func.sum(Payment.amount), 0))
        .filter(Payment.payment_date >= today_start, Payment.status == "completed", Payment.payment_type == "payment")
        .first()
    )
    ...
    week_stats = (
        db.query(func.count(Payment.id), func.coalesce(func.sum(Payment.amount), 0))
        .filter(Payment.payment_date >= week_start, Payment.status == "completed", Payment.payment_type == "payment")
        .first()
    )
    ...
    month_stats = (
        db.query(func.count(Payment.id), func.coalesce(func.sum(Payment.amount), 0))
        .filter(Payment.payment_date >= month_start, Payment.status == "completed", Payment.payment_type == "payment")
        .first()
    )
...
async def get_order_payment_summary(
    ...
    # Get payment totals
    payments = db.query(Payment).filter(Payment.sales_order_id == order_id, Payment.status == "completed").all()
    ...
    last_payment = (
        db.query(Payment)
        .filter(Payment.sales_order_id == order_id, Payment.status == "completed", Payment.payment_type == "payment")
...
async def get_payment(
    ...
    payment = (
        db.query(Payment)
        .options(joinedload(Payment.sales_order), joinedload(Payment.recorded_by))
