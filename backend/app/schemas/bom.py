...
class BOMLineBase(BaseModel):
    ...
    consume_stage: Literal["production", "shipping"] = Field(
        "production", description="When to consume: 'production' or 'shipping'"
    )
...
class BOMLineUpdate(BaseModel):
    ...
    consume_stage: Optional[Literal["production", "shipping"]] = Field(
        None, description="When to consume: 'production' or 'shipping'"
    )
