...
def trigger_mrp_check(db: Session, sales_order_id: int, background: bool = False) -> Optional[dict]:
    """
    ...
    try:
        ...
        logger.info(
            f"MRP check requested for sales order {sales_order.order_number}", extra={"sales_order_id": sales_order_id}
        )

        return {"sales_order_id": sales_order_id, "status": "checked", "message": "MRP check completed"}
...
def trigger_mrp_recalculation(
    db: Session, context_id: int, reason: str, product_ids: Optional[List[int]] = None
) -> Optional[dict]:
    """
    ...
    try:
        ...
        logger.info(
            ...
            extra={"context_id": context_id, "reason": reason, "product_ids": product_ids},
        )
        ...
        logger.error(
            ...
            exc_info=True,
            extra={"context_id": context_id, "reason": reason, "error": str(e)},
        )
        # Don't raise - graceful degradation
        return {"error": str(e)}


def trigger_incremental_mrp(db: Session, product_ids: List[int]) -> Optional[dict]:
    """
    ...
    try:
        logger.info(f"Incremental MRP requested for {len(product_ids)} products", extra={"product_ids": product_ids})
        ...
        logger.error(
            f"Error in incremental MRP: {str(e)}", exc_info=True, extra={"product_ids": product_ids, "error": str(e)}
        )
        # Don't raise - graceful degradation
        return {"error": str(e)}
