...
class ProductionExecutionService:
    ...
    def get_bom_for_production_order(po: ProductionOrder, db: Session) -> Optional[BOM]:
        ...
        if po.bom_id:
            ...
            return db.query(BOM).filter(BOM.product_id == po.product_id, BOM.active.is_(True)).first()
        return None
                    ...
                    lot_requirements.append(
                        {
                            ...
                            "lot_required": True,
                            "reason": "Customer traceability requirement"
                            if customer_id
                            else "Global policy (material type)",
                        }
                    )
    ...
    def consume_production_stage_materials(
        po: ProductionOrder, good_quantity: float, bad_quantity: float, db: Session, created_by: str = "system"
    ) -> List[Dict[str, Any]]:
        """
        ...
        for res_txn in reservation_txns:
            ...
            if inventory:
                ...
                try:
                    ...
                    logger.warning(f"Inventory transaction requires approval for PO {po.code}: {str(e)}")
    ...
    def produce_finished_goods(
        ...
        inventory = (
            db.query(Inventory)
            .filter(Inventory.product_id == po.product_id, Inventory.location_id == location_id)
            .first()
        )
