...
from app.models import Product, BOM, Inventory, ProductionOrder, PurchaseOrder, PurchaseOrderLine, MRPRun, PlannedOrder
...
class MRPService:
    ...
    def run_mrp(
        ...
        try:
            ...
            production_orders = self._get_production_orders(horizon_date, include_draft_orders)
            ...
            if settings.INCLUDE_SALES_ORDERS_IN_MRP:
                try:
                    ...
                    for so in sales_orders:
                        ...
                        if so.order_type == "quote_based" and so.product_id:
                            ...
                            requirements = self.explode_bom(
                                ...
                                due_date=so.estimated_completion_date.date()
                            ...
                            lines = self.db.query(SalesOrderLine).filter(SalesOrderLine.sales_order_id == so.id).all()
                    ...
                    shipping_requirements = self._get_shipping_material_requirements(sales_orders, horizon_date)
                    ...
                    logger.error(
                        f"Error including Sales Orders in MRP: {str(e)}", exc_info=True, extra={"error": str(e)}
                    )
            ...
            net_requirements = self.calculate_net_requirements(list(all_requirements.values()))
            ...
            for shortage in shortages:
                ...
                if (
                    settings.MRP_ENABLE_SUB_ASSEMBLY_CASCADING
                    and original_req
                    and original_req.parent_product_id
                    and shortage.has_bom
                ):
    ...
    def explode_bom(
        ...
        bom = self.db.query(BOM).filter(BOM.product_id == product_id, BOM.active.is_(True)).first()
        ...
        try:
            ...
            routing = (
                self.db.query(Routing).filter(Routing.product_id == product_id, Routing.is_active.is_(True)).first()
            )

            if routing:
                # Check if any operations have materials defined
                routing_material_count = (
                    self.db.query(RoutingOperationMaterial)
        ...
        if routing and has_routing_materials:
            ...
            operations = self.db.query(RoutingOperation).filter(RoutingOperation.routing_id == routing.id).all()
    ...
    def calculate_net_requirements(
        self, requirements: List[ComponentRequirement], source_production_order_ids: Optional[set] = None
    ) -> List[NetRequirement]:
        """
        ...
        if source_production_order_ids:
            allocations_by_order = get_allocations_by_production_order(self.db, product_ids)
        ...
        for req in requirements:
            ...
            inv = inventory_by_product.get(
                req.product_id, {"on_hand": Decimal("0"), "allocated": Decimal("0"), "available": Decimal("0")}
            )
    ...
    def generate_planned_orders(
        ...
        for shortage in shortages:
            ...
            if settings.MRP_ENABLE_SUB_ASSEMBLY_CASCADING and parent_due_date and order_type == "production":
    ...
    def _get_production_orders(self, horizon_date: date, include_draft: bool) -> List[ProductionOrder]:
    ...
    def _get_inventory_levels(self, product_ids: List[int]) -> Dict[int, Dict[str, Decimal]]:
        ...
        for row in inventory_query:
            ...
            result[row.product_id] = {"on_hand": on_hand, "allocated": allocated, "available": on_hand - allocated}

        return result

    def _get_incoming_supply(self, product_ids: List[int]) -> Dict[int, Decimal]:
        ...
        po_lines = (
            self.db.query(
                ...
                func.sum(PurchaseOrderLine.quantity_ordered - PurchaseOrderLine.quantity_received).label("incoming"),
            )
            .join(PurchaseOrder)
            .filter(PurchaseOrderLine.product_id.in_(product_ids), PurchaseOrder.status.in_(open_statuses))
    ...
    def _delete_unfirmed_planned_orders(self):
        ...
        self.db.query(PlannedOrder).filter(PlannedOrder.status == "planned").delete()
        self.db.flush()

    def _get_shipping_material_requirements(self, sales_orders: List, horizon_date: date) -> List[ComponentRequirement]:
        """
        ...
        for so in sales_orders:
            ...
            for product_id, qty in products_to_ship:
                ...
                shipping_lines = (
                    self.db.query(BOMLine).filter(BOMLine.bom_id == bom.id, BOMLine.consume_stage == "shipping").all()
                )
    ...
    def _get_sales_orders_within_horizon(self, horizon_date: date) -> List:
        """
        ...
        so_ids_with_po = (
            self.db.query(ProductionOrder.sales_order_id).filter(ProductionOrder.sales_order_id.isnot(None)).distinct()
        )

        # Get orders that are not cancelled, within horizon, and don't have linked POs
        query = self.db.query(SalesOrder).filter(SalesOrder.status != "cancelled", ~SalesOrder.id.in_(so_ids_with_po))
        ...
        query = query.filter(
            or_(
                ...
                and_(SalesOrder.estimated_completion_date.is_(None), SalesOrder.created_at <= horizon_datetime),
            )
        )
        ...
        for order in orders:
            if order.order_type == "quote_based" and order.product_id:
                ...
                lines = (
                    ...
                    .filter(SalesOrderLine.sales_order_id == order.id, SalesOrderLine.product_id.isnot(None))
                    .count()
                )
                if lines > 0:
                    result.append(order)
    ...
    def firm_planned_order(
        self, planned_order_id: int, user_id: Optional[int] = None, notes: Optional[str] = None
    ) -> PlannedOrder:
        """
    ...
    def _create_purchase_order(self, planned_order: PlannedOrder, vendor_id: int, user_id: Optional[int]) -> int:
    ...
    def _create_production_order(self, planned_order: PlannedOrder, user_id: Optional[int]) -> int:
        ...
        last_po = (
            ...
            .order_by(ProductionOrder.code.desc())
            .first()
        )
        ...
        bom = self.db.query(BOM).filter(BOM.product_id == planned_order.product_id, BOM.active.is_(True)).first()
    ...
    def get_supply_demand_timeline(self, product_id: int, days_ahead: int = 30) -> Dict:
        """
        ...
        inv_total = (
            self.db.query(func.sum(Inventory.on_hand_quantity), func.sum(Inventory.allocated_quantity))
        ...
        daily_demand = sum(e["quantity"] for e in entries if e["entry_type"] == "demand")
