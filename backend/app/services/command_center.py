...
def get_action_items(db: Session) -> ActionItemsResponse:
    ...
    return ActionItemsResponse(items=items, total_count=len(items), counts_by_type=dict(counts_by_type))
...
def _get_blocked_production_orders(db: Session) -> List[ActionItem]:
    ...
    active_pos = (
        db.query(ProductionOrder).filter(ProductionOrder.status.in_(["released", "in_progress", "scheduled"])).all()
    )
    ...
    for po in active_pos:
        ...
        if blocking and not blocking.status_summary.can_produce:
            ...
            suggested = [SuggestedAction(label="View Order", url=f"/admin/production/{po.id}", action_type="navigate")]
            ...
            items.append(
                ActionItem(
                    ...
                    metadata={"blocking_count": str(blocking.status_summary.blocking_count), "status": po.status},
                )
            )
        ...
        items.append(
            ActionItem(
                ...
                suggested_actions=[
                    ...
                    SuggestedAction(
                        label="View Production", url=f"/admin/production?so_id={so.id}", action_type="navigate"
                    ),
                ],
...
def _get_overrunning_operations(db: Session) -> List[ActionItem]:
    ...
    running_ops = (
        ...
        .filter(ProductionOrderOperation.status == "running", ProductionOrderOperation.actual_start.isnot(None))
        .all()
    )
    ...
    for op in running_ops:
        ...
        if planned_minutes > 0 and elapsed_minutes > (planned_minutes * 2):
            ...
            po = db.query(ProductionOrder).filter(ProductionOrder.id == op.production_order_id).first()
...
def _get_idle_resources_with_work(db: Session) -> List[ActionItem]:
    ...
    resources = db.query(Resource).filter(Resource.is_active.is_(True), Resource.status == "available").all()

    for resource in resources:
        # Check if resource has any running operations
        running_op = (
            db.query(ProductionOrderOperation)
            .filter(ProductionOrderOperation.resource_id == resource.id, ProductionOrderOperation.status == "running")
            .first()
        )
        ...
        if pending_count > 0:
            wc = db.query(WorkCenter).filter(WorkCenter.id == resource.work_center_id).first()
            ...
            items.append(
                ActionItem(
                    ...
                    metadata={"pending_count": str(pending_count), "work_center": wc.name if wc else ""},
                )
            )
...
def get_today_summary(db: Session) -> TodaySummary:
    ...
    orders_shipped_today = (
        db.query(SalesOrder).filter(SalesOrder.shipped_at >= today_start, SalesOrder.shipped_at < today_end).count()
    )
    ...
    operations_running = db.query(ProductionOrderOperation).filter(ProductionOrderOperation.status == "running").count()

    # Resource counts
    resources_total = db.query(Resource).filter(Resource.is_active.is_(True)).count()

    # Busy = has running operation
    resources_with_running_ops = (
        db.query(func.count(func.distinct(ProductionOrderOperation.resource_id)))
        .filter(ProductionOrderOperation.status == "running", ProductionOrderOperation.resource_id.isnot(None))
        .scalar()
        or 0
    )
    ...
    resources_down = (
        db.query(Resource).filter(Resource.is_active.is_(True), Resource.status.in_(["maintenance", "offline"])).count()
    )
...
def get_resource_statuses(db: Session) -> ResourcesResponse:
    ...
    resources = (
        db.query(Resource).filter(Resource.is_active.is_(True)).order_by(Resource.work_center_id, Resource.code).all()
    )
    ...
    for resource in resources:
        ...
        if running_op:
            status = "running"
            # Get production order for context
            po = db.query(ProductionOrder).filter(ProductionOrder.id == running_op.production_order_id).first()
            product = None
            if po:
    ...
    return ResourcesResponse(resources=result_resources, summary=dict(status_counts))
