...
class OrderStatusService:
    ...
    def validate_so_transition(self, from_status: str, to_status: str) -> Tuple[bool, str]:
        ...
        if to_status not in valid_next:
            return (
                False,
                f"Invalid SO status transition: '{from_status}' → '{to_status}'. Valid options: {', '.join(valid_next)}",
            )
    ...
    def validate_wo_transition(self, from_status: str, to_status: str) -> Tuple[bool, str]:
        ...
        if to_status not in valid_next:
            return (
                False,
                f"Invalid WO status transition: '{from_status}' → '{to_status}'. Valid options: {', '.join(valid_next)}",
            )
    ...
    def update_so_status(
        self, db: Session, so: SalesOrder, new_status: str, skip_validation: bool = False
    ) -> SalesOrder:
        """
    ...
    def auto_update_so_from_wos(self, db: Session, so: SalesOrder) -> SalesOrder:
        ...
        wos = db.query(ProductionOrder).filter(ProductionOrder.sales_order_id == so.id).all()
    ...
    def update_wo_status(
        self, db: Session, wo: ProductionOrder, new_status: str, skip_validation: bool = False
    ) -> ProductionOrder:
        """
    ...
    def scrap_wo_and_create_remake(
        self, db: Session, wo: ProductionOrder, scrap_reason: str, scrap_quantity: Optional[float] = None
    ) -> ProductionOrder:
        """
        ...
        existing_remakes = db.query(ProductionOrder).filter(ProductionOrder.remake_of_id == wo.id).count()
    ...
    def mark_ready_for_shipping(self, db: Session, so: SalesOrder) -> SalesOrder:
        ...
        if incomplete:
            raise ValueError(f"Cannot mark ready to ship: {len(incomplete)} WOs still in progress")

        failed_qc = [wo for wo in wos if wo.qc_status == "failed"]
        if failed_qc:
            raise ValueError(f"Cannot mark ready to ship: {len(failed_qc)} WOs failed QC")
