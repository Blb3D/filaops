...
class LotPolicyService:
    ...
    def is_lot_required_for_product(
        ...
        if sales_order_id:
            ...
            if sales_order:
                ...
                customer_profile = (
                    ...
                    .filter(CustomerTraceabilityProfile.user_id == sales_order.customer_id)
                    .first()
                )
        ...
        if customer_id:
            customer_profile = (
                db.query(CustomerTraceabilityProfile).filter(CustomerTraceabilityProfile.user_id == customer_id).first()
            )
        ...
        if product_type and product_type.lower() in LotPolicyService.REQUIRED_PRODUCT_TYPES:
            logger.info(f"Lot required for product {product.sku} due to product type: {product_type}")
            return True
    ...
    @staticmethod
    def get_required_lots_for_production_order(production_order_id: int, db: Session) -> List[Dict[str, Any]]:
        """
