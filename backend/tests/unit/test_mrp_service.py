...
@pytest.fixture
def inventory_with_stock(db, inventory_location, raw_material_pla, raw_material_petg, packaging_box, hardware_insert):
...
class TestBOMExplosion:
    ...
    def test_multi_level_bom_explosion(
        self,
        db,
        mrp_service,
...
class TestNetRequirements:
    ...
    def test_net_requirement_with_sufficient_stock(self, db, mrp_service, raw_material_pla, inventory_with_stock):
    ...
    def test_net_requirement_with_shortage(self, db, mrp_service, raw_material_pla, inventory_with_stock):
    ...
    def test_aggregates_multiple_requirements(
        self, db, mrp_service, raw_material_pla, raw_material_petg, inventory_with_stock
    ):
...
class TestPlannedOrderGeneration:
    ...
    def test_generates_production_order_for_make_item(self, db, mrp_service, finished_product_simple):
...
class TestFullMRPRun:
    ...
    def test_full_mrp_run_creates_planned_orders(
        self, db, mrp_service, finished_product_multilevel, inventory_with_stock, inventory_location
    ):
...
class TestEdgeCases:
    ...
    def test_inactive_bom_not_exploded(self, db, mrp_service, finished_product_simple):
        ...
        # Deactivate the BOM
        bom = db.query(BOM).filter(BOM.product_id == finished_product_simple.id).first()
        bom.active = False
        db.commit()
