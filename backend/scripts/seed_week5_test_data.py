...
def seed_data():
    ...
    try:
        ...
        if not res_finish:
            res_finish = Machine(
                code="FINISH-01", name="Finishing Station", work_center_id=wc_finish.id, status="available"
            )
            db.add(res_finish)
        ...
        if not prod1:
            ...
            routing1 = Routing(
                product_id=prod1.id, code="RTG-WIDGET-TEST-001", name="Widget Test Routing", is_active=True
            )
            db.add(routing1)
            db.flush()
            ...
            db.add_all(
                [
                    ...
                    RoutingOperation(
                        ...
                        sequence=20,
                        operation_code="PACK",
                        operation_name="Package",
                        work_center_id=wc_ship.id,
        ...
        if not prod2:
            ...
            routing2 = Routing(
                product_id=prod2.id, code="RTG-GADGET-TEST-PRO", name="Gadget Pro Test Routing", is_active=True
            )
            db.add(routing2)
            db.flush()
            ...
            db.add_all(
                [
                    RoutingOperation(
                        routing_id=routing2.id,
                        sequence=10,
                        operation_code="PRINT",
                        operation_name="3D Print Components",
                        work_center_id=wc_print.id,
                    ...
                    RoutingOperation(
                        ...
                        operation_name="Post-Print Cleanup",
                        work_center_id=wc_finish.id,
                        setup_time_minutes=5,
                        run_time_minutes=15,
                    ),
                    RoutingOperation(
        ...
        po1 = ProductionOrder(code="PO-TEST-001", product_id=prod1.id, quantity_ordered=5, status="draft", priority=3)
        db.add(po1)

        # --- PO 2: Released with operations (all pending) ---
        print("  Creating PO-TEST-002: Released, all pending...")
        po2 = ProductionOrder(
            code="PO-TEST-002", product_id=prod2.id, quantity_ordered=10, status="released", priority=1
        )
        db.add(po2)
        db.flush()
        ...
        db.add_all(
            [
                ProductionOrderOperation(
                    ...
                    operation_name="3D Print Components",
                    work_center_id=wc_print.id,
                    planned_setup_minutes=10,
                    planned_run_minutes=1200,
                    status="pending",
                ),
                ProductionOrderOperation(
                    production_order_id=po2.id,
                ...
                ProductionOrderOperation(
                    ...
                    operation_name="Quality Inspection",
                    work_center_id=wc_qc.id,
                    planned_setup_minutes=2,
                    planned_run_minutes=100,
                    status="pending",
                ),
                ProductionOrderOperation(
                    production_order_id=po2.id,
                    ...
                    operation_name="Pack & Label",
                    work_center_id=wc_ship.id,
                    planned_setup_minutes=3,
                    planned_run_minutes=80,
                    status="pending",
                ),
            ]
        )
        ...
        po3 = ProductionOrder(
            code="PO-TEST-003", product_id=prod2.id, quantity_ordered=5, status="in_progress", priority=1
        )
        db.add(po3)
        db.flush()
        ...
        db.add_all(
            [
                ProductionOrderOperation(
                    ...
                    planned_setup_minutes=10,
                    planned_run_minutes=600,
                    actual_setup_minutes=8,
                    actual_run_minutes=580,
        ...
        po4 = ProductionOrder(
            code="PO-TEST-004", product_id=prod2.id, quantity_ordered=3, status="in_progress", priority=3
        )
        db.add(po4)
        db.flush()
        ...
        db.add_all(
            [
                ProductionOrderOperation(
                    ...
                    actual_setup_minutes=12,
                    actual_run_minutes=340,
                ...
                ProductionOrderOperation(
                    ...
                    resource_id=res_finish.id,
                    planned_setup_minutes=5,
                    planned_run_minutes=45,
                    actual_setup_minutes=5,
                    actual_run_minutes=38,
        ...
        db.add_all(
            [
                ProductionOrderOperation(
                    ...
                    planned_setup_minutes=5,
                    planned_run_minutes=900,
                    actual_setup_minutes=5,
                    actual_run_minutes=875,
