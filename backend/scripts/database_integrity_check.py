...
from app.models import Product, BOM, BOMLine, Inventory, ProductionOrder, PurchaseOrderLine, PlannedOrder, ItemCategory
...
class DatabaseIntegrityChecker:
    ...
    def check_missing_categories(self):
        ...
        products_without_category = self.db.query(Product).filter(Product.category_id.is_(None)).all()
    ...
    def check_orphaned_products(self):
        ...
        orphaned_bom_components = (
            self.db.query(BOMLine)
            .join(Product, BOMLine.component_id == Product.id, isouter=True)
    ...
    def check_invalid_bom_references(self):
        ...
        empty_boms = self.db.query(BOM).outerjoin(BOMLine).filter(BOMLine.id.is_(None)).all()

        if empty_boms:
            self.issues_found.append({"type": "empty_boms", "count": len(empty_boms), "boms": empty_boms})
            print(f"   ⚠️  {len(empty_boms)} BOMs have no lines")

        # Products marked has_bom=True but no active BOM
        products_missing_boms = (
            self.db.query(Product)
            .filter(Product.has_bom == True)
            .outerjoin(BOM, (BOM.product_id == Product.id) & (BOM.active == True))
            .filter(BOM.id.is_(None))
            .all()
        )

        if products_missing_boms:
            self.issues_found.append(
                {"type": "missing_boms", "count": len(products_missing_boms), "products": products_missing_boms}
            )
    ...
    def check_inventory_inconsistencies(self):
        ...
        negative_inventory = self.db.query(Inventory).filter(Inventory.on_hand_quantity < 0).all()

        if negative_inventory:
            issues.append(f"{len(negative_inventory)} negative on-hand quantities")
            self.issues_found.append(
                {"type": "negative_inventory", "count": len(negative_inventory), "records": negative_inventory}
            )

        # Allocated > On-hand
        over_allocated = (
            self.db.query(Inventory).filter(Inventory.allocated_quantity > Inventory.on_hand_quantity).all()
        )

        if over_allocated:
            issues.append(f"{len(over_allocated)} over-allocated inventory")
            self.issues_found.append(
                {"type": "over_allocated", "count": len(over_allocated), "records": over_allocated}
            )
        ...
        if orphaned_inventory:
            ...
            self.issues_found.append(
                {"type": "orphaned_inventory", "count": len(orphaned_inventory), "records": orphaned_inventory}
            )
    ...
    def check_production_order_integrity(self):
        ...
        over_completed = (
            ...
            .filter(ProductionOrder.quantity_completed > ProductionOrder.quantity_ordered)
            .all()
        )
        ...
        if over_completed:
            ...
            self.issues_found.append(
                {"type": "over_completed_po", "count": len(over_completed), "orders": over_completed}
            )

        # Production orders for non-existent products
        orphaned_pos = (
            self.db.query(ProductionOrder)
            .join(Product, ProductionOrder.product_id == Product.id, isouter=True)
            .filter(Product.id.is_(None))
            .all()
        )
        ...
        if orphaned_pos:
            ...
            self.issues_found.append(
                {"type": "orphaned_production_orders", "count": len(orphaned_pos), "orders": orphaned_pos}
            )

        # Invalid status combinations
        invalid_status = (
            self.db.query(ProductionOrder)
            .filter((ProductionOrder.status == "completed") & (ProductionOrder.actual_completion.is_(None)))
            .all()
        )
        ...
        if invalid_status:
            ...
            self.issues_found.append(
                {"type": "invalid_po_status", "count": len(invalid_status), "orders": invalid_status}
            )
    ...
    def check_purchase_order_integrity(self):
        ...
        over_received = (
            ...
            .filter(PurchaseOrderLine.quantity_received > PurchaseOrderLine.quantity_ordered)
            .all()
        )
        ...
        if over_received:
            ...
            self.issues_found.append(
                {"type": "over_received_po_lines", "count": len(over_received), "lines": over_received}
            )

        # PO lines for non-existent products
        orphaned_po_lines = (
            self.db.query(PurchaseOrderLine)
            .join(Product, PurchaseOrderLine.product_id == Product.id, isouter=True)
            .filter(Product.id.is_(None))
            .all()
        )
        ...
        if orphaned_po_lines:
            ...
            self.issues_found.append(
                {"type": "orphaned_po_lines", "count": len(orphaned_po_lines), "lines": orphaned_po_lines}
            )
    ...
    def check_planned_order_cleanup(self):
        ...
        stale_planned = (
            ...
            .join(Product, PlannedOrder.product_id == Product.id, isouter=True)
            .filter(Product.id.is_(None))
            .all()
        )
        ...
        if stale_planned:
            self.issues_found.append(
                {"type": "stale_planned_orders", "count": len(stale_planned), "orders": stale_planned}
            )
    ...
    def check_uom_consistency(self):
        ...
        missing_units = self.db.query(Product).filter((Product.unit.is_(None)) | (Product.unit == "")).all()

        if missing_units:
            issues.append(f"{len(missing_units)} products missing units")
            self.issues_found.append({"type": "missing_uom", "count": len(missing_units), "products": missing_units})

        # BOM lines with mismatched units (advanced check)
        bom_lines_unit_mismatch = (
            self.db.query(BOMLine)
            .join(Product, BOMLine.component_id == Product.id)
            .filter((BOMLine.unit != Product.unit) & (BOMLine.unit.isnot(None)) & (Product.unit.isnot(None)))
            .all()
        )
        ...
        if bom_lines_unit_mismatch:
            print(
                f"   ℹ️  {len(bom_lines_unit_mismatch)} BOM lines use different units than component base unit (this may be intentional)"
            )
    ...
    def check_quantity_anomalies(self):
        ...
        large_inventory = self.db.query(Inventory).filter(Inventory.on_hand_quantity > 1000000).all()
        ...
        zero_cost_products = (
            self.db.query(Product)
            .filter((Product.standard_cost <= 0) & (Product.item_type.in_(["raw_material", "component"])))
            .all()
        )
        ...
        if zero_cost_products:
            ...
            self.issues_found.append(
                {"type": "zero_cost_products", "count": len(zero_cost_products), "products": zero_cost_products}
            )
    ...
    def check_database_statistics(self):
        ...
        try:
            ...
            if inventory_count > 50000:
                print("   ⚠️  Large inventory table may impact MRP performance")

            if bom_count > product_count * 0.8:
                print("   ⚠️  High BOM ratio may indicate deep/complex product structures")
    ...
    def _repair_missing_categories(self, issue):
        ...
        default_category = self.db.query(ItemCategory).filter(ItemCategory.name == "Uncategorized").first()

        if not default_category:
            default_category = ItemCategory(
                name="Uncategorized", description="Auto-created for products without categories", created_by="system"
            )
